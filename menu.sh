...
start_loop() {
  ...
  printf '[%s] ===== START loop docker detached =====
' "$(now)" >> "$LOOP_LOG"

  rm -f "$BASE_DIR/loop.pid" 2>/dev/null || true

  # Start loop lepas dari terminal Firebase:
  # setsid + nohup + stdin /dev/null = tetap jalan walau Firebase refresh/disconnect
  cd "$CODE_DIR" || return
  setsid nohup env PYTHONUNBUFFERED=1 xvfb-run -a --server-args="-screen 0 ${SCREEN_LOOP}" \
    "$PYTHON_BIN" -u "$LOOP_FILE" >> "$LOOP_LOG" 2>&1 < /dev/null &
  ...
  if proc_running "[l]oop.py"; then
    log_line "START loop docker detached"
    LAST_MSG="Loop started. Aman walau Firebase disconnect/refresh."
  else
    log_line "FAILED START loop docker detached"
    LAST_MSG="Loop gagal start. Cek loop_log.txt"
  fi
}
...
stop_loop() {
  if [ -f "$BASE_DIR/loop.pid" ]; then
    pid="$(cat "$BASE_DIR/loop.pid" 2>/dev/null || true)"
    if [ -n "$pid" ]; then
...
cleanup_root_png() {
  find "$BASE_DIR/snapshots" -type f \( -name '*.png' -o -name '*.jpg' -o -name '*.jpeg' \) -delete 2>/dev/null || true
  log_line "CLEANUP snapshots"
  LAST_MSG="Semua screenshot di folder snapshots dibersihkan."
}

cleanup_logs_json_lock() {
  find "$BASE_DIR" -maxdepth 2 -type f \( -name '*.log' -o -name '*.lock' \) -delete 2>/dev/null || true
  touch "$BOT_LOG" "$LOGIN_LOG" "$LOOP_LOG" "$BUAT_LINK_LOG"
  log_line "CLEANUP log/lock"
  LAST_MSG="Semua file log dan .lock dibersihkan."
}
...
view_logs() {
  while true; do
    ...
    echo "===== BOT LOG ====="
    tail -n 25 "$BOT_LOG" 2>/dev/null || true
    ...
    echo
    echo "===== LOOP LOG ====="
    tail -n 20 "$LOOP_LOG" 2>/dev/null || true

    echo
    echo "===== BUAT LINK LOG ====="
    tail -n 20 "$BUAT_LINK_LOG" 2>/dev/null || true

    echo
    echo "[ Tekan Q untuk kembali ]"
